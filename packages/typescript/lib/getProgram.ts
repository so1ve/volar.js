...
import type { createProject } from '@volar/language-core';
...
export function getProgram(
	...
	function getSourceFileDiagnosticsWorker<T extends 'getSyntacticDiagnostics' | 'getSemanticDiagnostics'>(
		...
		if (sourceFile) {
			...
			const [virtualFile, source] = project.virtualFiles.getVirtualFile(sourceFile.fileName);
	...
	function transformDiagnostics<T extends ts.Diagnostic | ts.DiagnosticWithLocation | ts.DiagnosticRelatedInformation>(diagnostics: readonly T[]): T[] {
		...
		for (const diagnostic of diagnostics) {
			if (
				...
				const [virtualFile, source] = project.virtualFiles.getVirtualFile(diagnostic.file.fileName);
				...
				if (virtualFile && source) {
					...
					for (const [_, [sourceSnapshot, map]] of project.virtualFiles.getMaps(virtualFile)) {
		...
		function onMapping(diagnostic: T, fileName: string, start: number, end: number, docText: string | undefined) {
			...
			if (!file) {
				...
				if (docText === undefined) {
					const snapshot = project.resolvedHost.getScriptSnapshot(fileName);
					if (snapshot) {
