	...
	function createDocMap(file: VirtualFile, _sourceFileName: string, _sourceSnapshot: ts.IScriptSnapshot) {
		...
		if (maps.has(_sourceFileName) && maps.get(_sourceFileName)![0] === _sourceSnapshot) {
			...
			return new SourceMapWithDocuments(
				TextDocument.create(context.env.fileNameToUri(_sourceFileName), context.project.resolvedHost.getLanguageId?.(_sourceFileName) ?? resolveCommonLanguageId(context.env.fileNameToUri(_sourceFileName)), version, _sourceSnapshot.getText(0, _sourceSnapshot.getLength())),
				TextDocument.create(context.env.fileNameToUri(file.fileName), context.project.resolvedHost.getLanguageId?.(file.fileName) ?? resolveCommonLanguageId(context.env.fileNameToUri(file.fileName)), version, file.snapshot.getText(0, file.snapshot.getLength())),
				map,
			);
		}
	}
