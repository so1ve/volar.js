...
export function register(context: ServiceContext) {
	...
	return async (
		...
		const newSnapshot = context.project.resolvedHost.getScriptSnapshot(context.env.uriToFileName(uri));
		...
		async function lintWorker(
			...
			const result = await ruleWorker(
				...
				async (ruleId, rule, lintDocument, ruleCtx) => {
					...
					const projectVersion = (ruleType === RuleType.Semantic) ? context.project.resolvedHost.getProjectVersion?.() : undefined;
		...
		async function worker(
			...
			const result = await languageFeatureWorker(
				...
				async (service, document) => {
					...
					const projectVersion = api === 'provideSemanticDiagnostics' ? context.project.resolvedHost.getProjectVersion?.() : undefined;
